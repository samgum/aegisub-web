...
test("desktop download stays visible and opens the moving latest-release link without losing edits", async ({ page, context }, info) => {
  ...
  const [popup] = await Promise.all([context.waitForEvent("page"), /android|ipad|iphone/.test(info.project.name) ? link.tap() : link.click()]);
  ...
  await popup.close(); await page.bringToFront(); await page.screenshot({ path: info.outputPath("desktop-download.png") });
});
